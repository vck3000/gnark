...
func BenchmarkSetup(b *testing.B) {
	...
	b.Run("setup", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			bls24_315groth16.Setup(ccs.(*cs.R1CS), &pk, &vk)
		}
	})
}
...
func BenchmarkProver(b *testing.B) {
	...
	var pk bls24_315groth16.ProvingKey
	bls24_315groth16.DummySetup(ccs.(*cs.R1CS), &pk)

	b.ResetTimer()
	b.Run("prover", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, _ = bls24_315groth16.Prove(ccs.(*cs.R1CS), &pk, fullWitness, backend.ProverOption{})
		}
	})
}
...
func BenchmarkVerifier(b *testing.B) {
	...
	var vk bls24_315groth16.VerifyingKey
	bls24_315groth16.Setup(ccs.(*cs.R1CS), &pk, &vk)
	proof, err := bls24_315groth16.Prove(ccs.(*cs.R1CS), &pk, fullWitness, backend.ProverOption{})
	if err != nil {
		panic(err)
	}
