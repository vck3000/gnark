...
func BenchmarkSetup(b *testing.B) {
	...
	b.Run("setup", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			bw6_633groth16.Setup(ccs.(*cs.R1CS), &pk, &vk)
		}
	})
}
...
func BenchmarkProver(b *testing.B) {
	...
	var pk bw6_633groth16.ProvingKey
	bw6_633groth16.DummySetup(ccs.(*cs.R1CS), &pk)

	b.ResetTimer()
	b.Run("prover", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, _ = bw6_633groth16.Prove(ccs.(*cs.R1CS), &pk, fullWitness, backend.ProverOption{})
		}
	})
}
...
func BenchmarkVerifier(b *testing.B) {
	...
	var vk bw6_633groth16.VerifyingKey
	bw6_633groth16.Setup(ccs.(*cs.R1CS), &pk, &vk)
	proof, err := bw6_633groth16.Prove(ccs.(*cs.R1CS), &pk, fullWitness, backend.ProverOption{})
	if err != nil {
		panic(err)
	}
