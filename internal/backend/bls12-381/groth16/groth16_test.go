...
func referenceCircuit() (compiled.ConstraintSystem, frontend.Circuit) {
	...
	ccs, err := frontend.Compile(r1cs.Builder(curve.ID), &circuit)
	if err != nil {
		panic(err)
	}
...
func BenchmarkSetup(b *testing.B) {
	...
	b.Run("setup", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			bls12_381groth16.Setup(ccs.(*cs.R1CS), &pk, &vk)
		}
	})
}

func BenchmarkProver(b *testing.B) {
	ccs, _solution := referenceCircuit()
	...
	var pk bls12_381groth16.ProvingKey
	bls12_381groth16.DummySetup(ccs.(*cs.R1CS), &pk)

	b.ResetTimer()
	b.Run("prover", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, _ = bls12_381groth16.Prove(ccs.(*cs.R1CS), &pk, fullWitness, backend.ProverOption{})
		}
	})
}
...
func BenchmarkVerifier(b *testing.B) {
	...
	var vk bls12_381groth16.VerifyingKey
	bls12_381groth16.Setup(ccs.(*cs.R1CS), &pk, &vk)
	proof, err := bls12_381groth16.Prove(ccs.(*cs.R1CS), &pk, fullWitness, backend.ProverOption{})
	if err != nil {
		panic(err)
	}
